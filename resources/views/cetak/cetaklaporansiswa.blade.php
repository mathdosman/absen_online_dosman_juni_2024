...
<table class="tabeldatasiswa" style="padding: 1rem">
    <tr>
        <td rowspan="6">
            ...
            @else
            <img src="{{asset('assets/img/sample/avatar/avatar1.jpg')}}" alt="xxxx" width="80" height="100" class="border border-1 border-dark rounded ms-2">&emsp;&emsp;
            @endif
        </td>
    </tr>
...
$jam = date("H",strtotime($jamterlambat))*1;
$menit = date("i",strtotime($jamterlambat))*1;
$detik = date("s",strtotime($jamterlambat))*1;
@endphp
<tr>
    ...
    <td>{!!$d->jam_out !== null ? date("H:i",strtotime($d->jam_out)): '<span style="color:red">Belum Absen</span>'!!}</td>
    <td>
        @if ($d->foto_out != null)
    ...
    <td>
        @if ($d->jam_in > '07:30')
        <b class="fw-bold text-danger">Terlambat <br> {{$jam==null? "":$jam.' jam '}} {{$menit}} menit </b>
        @else
